...
"""Live test for the Grok-on-Vertex agent.

Runs the agent against a real ``xai/grok-4.6`` endpoint and checks that text
generation, tool calling (both the function call and its response), and
multi-turn memory all work. Pass ``--stream`` to exercise ``StreamingMode.SSE``.
Exits non-zero if any check fails, so it can be used as a smoke test.

    python contributing/samples/models/grok/run.py
    python contributing/samples/models/grok/run.py --stream
"""
...
async def _run(stream: bool) -> int:
  ...
  print(f"=== Grok 4.6 live test (stream={stream}) ===")
